...
public class CommentServiceTest extends IntegrationTestSupport {
	...
	void returnWrittenCommentWhenSaving() {
		...
		// when
		CommentResponse commentResponse = commentService.save(request, writerId);
	...
	void returnCommentListWhenFindByIssueId() {
		...
		commentService.save(request1, writerId1);
		commentService.save(request2, writerId2);
		commentService.save(request3, writerId1);
	...
	void modifyCommentByDifferentWriter() {
		...
		// when, then
		assertThatThrownBy(() -> commentService.modifyComment(request, commentResponse.getId(), writerId2))
			.isInstanceOf(ApiException.class)
			.hasMessage(CommentException.NOT_AUTHORIZED.getMessage());
	}
}
